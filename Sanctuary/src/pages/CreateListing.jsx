...
export default function Createlisting() {
    ...
    const handleImageSubmit = (e) =>{
        if (files.length > 0 && files.length + formData.imageUrls.length < 7){
            ...
            Promise.all(promises).then((urls) =>{
                ...
                setImageUploadError('Image upload failed ( limited 2mb for every image)');
                setUploading(false);
            });
        }else{
            setImageUploadError('you can only upload 6 limited image');
            setUploading(false);
        }
    };
    const storeImage = async (file) => {
        return new Promise((resolve, reject) => {
            ...
            (snapshot) => {
                const progcess = (snapshot.bytesTransferred / snapshot.totalBytes) * 100;
                console.log(`Upload is ${progcess}% done`);
            },
             (error) => {
                reject(error);
        ...
        <form className='flex flex-col sm:flex-row gap-4'>
            ...
            <div className='flex flex-col flex-1 gap-4'>
                ...
                <div className='flex gap-4'>
                    <input onChange={(e)=>setFiles(e.target.files)} className='p-3 border border-gray-500 rounded w-full' type="file" id='image' accept='image/*' multiple/>
                    <button type='button' disabled={uploading} onClick={handleImageSubmit} className='p-3 text-green-700 border border-green-700 rounded uppercase hover:shadow-lg disabled:opacity-80'> {uploading ? 'Uploading...': 'Uploading'}</button>
                ...
                <p className='text-red-700 text-sm'>{imageUploadError && imageUploadError}</p>
                {
                    formData.imageUrls.length > 0 && formData.imageUrls.map((url, index)=> (
                        ...
                        <button type='button' onClick={() => handleRemoveImage(index)} className='p-3 text-red-700 rounded-lg uppercase hover:opacity-80 disabled:opacity-80'>Delete</button>
                        </div>
                    ))
                }
            </div>
