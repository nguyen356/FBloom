...
export default function Profile() {
  ...
  const handleFileUpload = (file) => {
    ...
    ()=> {
      ...
      ((downloadURL)=>{
        setFormData({...formData, imageavatar:downloadURL});
      });
    },
    );
  };
  ...
  return (
    <div className='p-3 max-w-lg mx-auto'>
      ...
      <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
        ...
        <img onClick={()=>fileRef.current.click()} src={formData.imageavatar || currentUser.imageavatar} alt="profile" className='rounded-full h-30 w-30 object-cover cursor-pointer self-center mt-5'/>
        <p className='text-sm self-center'>
        {fileUploadError ? (<span className='text-red-700'>Error Image Upload... (image must be less than 2mb)</span>) 
        :
        filePerc > 0 && filePerc < 100 ? (<span className='text-black'>{`Uploading ${filePerc}%`}</span>)
        :
        filePerc === 100 ? (<span className='text-green-700'>File Succesfully Uploaded</span>)
        :     ('')
        }
        </p>
