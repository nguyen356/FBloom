...
export const google = async (req, res,next) => {
    try {
        ...
        if (user) {
            ...
            const newUser = new User({username:req.body.name.split(" ").join("").toLowerCase() + Math.random().toString(36).slice(-4), email: req.body.email, password:hashPassword, imageavatar: req.body.photo});
            await newUser.save();
